...
with SSHTunnelForwarder(('rcg-linux-ts1.rcg.sfu.ca', 22),
    ...
    for year in [2001, 2002, 2007, 2008]:
        ...
        cursor.execute("""Drop view if exists chao_draft.cal_m5p_rank_corr_CSS_null_norm_%s_view;""" % str(year))
        mydb.commit()
        cursor.execute("""Create view chao_draft.cal_m5p_rank_corr_CSS_null_norm_%s_view as
                    SELECT id, PlayerName, DraftYear, rank_sum_7yr_GP, 
                    skaters_overall, pow(skaters_overall-rank_sum_7yr_GP, 2) as di2_overall,          
                    rank_m5p_notie, pow(rank_m5p_notie - rank_sum_7yr_GP, 2) as di2_m5p_notie,
                    rank_m5p_tied, pow(rank_m5p_tied - rank_sum_7yr_GP, 2) as di2_m5p_tied,
                    rank_m5p_nonzero, pow(rank_m5p_nonzero - rank_sum_7yr_GP, 2) as di2_m5p_nonzero
                  FROM chao_draft.union_all_ranks_with_m5p_view
        ...
        cursor.execute("""select count(*), sum(di2_overall), sum(di2_m5p_notie), 
                        ...
                        from chao_draft.cal_m5p_rank_corr_CSS_null_norm_%s_view;""" % str(year))
        #mydb.commit()
    ...
    with open("m5p_rank_correlation_calculation.csv", 'wb') as output_file:
