...
with SSHTunnelForwarder(('rcg-linux-ts1.rcg.sfu.ca', 22),
    ...
    cursor.execute("Drop table if exists chao_draft.m5p_10years_CSS_null_norm_TOI_pred;")

    mydb.commit()

    cursor.execute("""Create table chao_draft.m5p_10years_CSS_null_norm_TOI_pred
    ...
    for file in os.listdir(inputDir):
        if file.endswith("all_5_year_norm_TOI_pred.csv"):
        ...
        with open(inputDir + "/" + fileName,'r') as inputFile:
            ...
            for row in csv_data:
                if firstLine:
                    firstLine = False
                    continue
                cursor.execute("""INSERT INTO chao_draft.m5p_10years_CSS_null_norm_TOI_pred
