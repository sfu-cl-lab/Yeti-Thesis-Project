...
with SSHTunnelForwarder(('rcg-linux-ts1.rcg.sfu.ca', 22),
    ...
    cursor.execute("Drop table if exists chao_draft.m5p_10years_nonzero_CSS_null_norm_TOI_pred;")

    mydb.commit()

    cursor.execute("""Create table chao_draft.m5p_10years_nonzero_CSS_null_norm_TOI_pred
    ...
    po_PIM_norm double, po_PlusMinus_norm double, sum_7yr_TOI int, m5p_pred double, LeafNode int);""")

    mydb.commit()

    # where unimported csv files are stored
    inputDir = '/Users/chao/GoogleDrive/2017-08-17_m5p_TOI/cross_validation'
    # csv files will be moved to hear after being imported to database
    moveToDir = '/Users/chao/GoogleDrive/2017-08-17_m5p_TOI/cross_validation'

    fileNameList = []
    for file in os.listdir(inputDir):
        if file.endswith("nonzero_5_year_norm_TOI_pred.csv"):
        ...
        with open(inputDir + "/" + fileName,'r') as inputFile:
            ...
            for row in csv_data:
                if firstLine:
                    firstLine = False
                    continue
                cursor.execute("""INSERT INTO chao_draft.m5p_10years_nonzero_CSS_null_norm_TOI_pred
                ...
                po_G_norm, po_A_norm, po_P_norm, po_PIM_norm, po_PlusMinus_norm, sum_7yr_TOI, m5p_pred, LeafNode) 
                VALUES(%s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s, 
