...
with SSHTunnelForwarder(('rcg-linux-ts1.rcg.sfu.ca', 22),
    ...
    cursor.execute("Drop table if exists chao_draft.lmt_10years_CSS_null_norm_prob;")
    ...
    po_PIM_norm double, po_PlusMinus_norm double, GP_greater_than_0 VARCHAR(10), wx_sum double,
     lmt_prob double, LeafNode int);""")
    ...
    for file in os.listdir(inputDir):
        if file.endswith("5_year_norm_prob.csv"):
        ...
        with open(inputDir + "/" + fileName,'r')  as inputFile:
            ...
            for row in csv_data:
                if firstLine:
                    firstLine = False
                    continue
                cursor.execute("""INSERT INTO chao_draft.lmt_10years_CSS_null_norm_prob
                ...
                rs_GP_norm, rs_G_norm, rs_A_norm, rs_P_norm, rs_PIM_norm, rs_PlusMinus_norm, po_GP_norm, po_G_norm, po_A_norm, po_P_norm,
                 po_PIM_norm, po_PlusMinus_norm, GP_greater_than_0, wx_sum, lmt_prob, LeafNode) VALUES(%s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s,
                                        %s, %s, %s, %s, %s, 
                                        %s, %s, %s, %s, %s)""", row)
